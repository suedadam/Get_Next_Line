...
int				main(void)
{
	...
	if (fd1 > 2 && fd2 > 2)
	{
		...
		{
			printf("Passed! The returned string is: \"%s\"\n", line1);
		}
		...
		ret1 = get_next_line(fd1, &line1);
		printf("(%d) Returned string = \"%s\"\n", ret1, line1);

		// ret1 = get_next_line(fd1, &line1);
		// if (ret1 != 1 || strcmp(line1, "") != 0)
		// {
		// 	printf("-> must have returned '1' and read line #5 \"\" from file %s - You returned \"%s\" (%d)\n", filename1, line1, ret1);
		// 	errors++;
		// }
		// else
		// {
